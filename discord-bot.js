        ...
        try {
            switch (commandName) {
                ...
                case 'managecode':
                    ...
                    if (subcommand === 'list') {
                        const userCodeMap = this.userCode.get(targetUser.id);

                        if (!userCodeMap || userCodeMap.size === 0) {
                            await interaction.reply({
                                content: `📁 No code files found for user **${targetUser.username}**.`,
                                ephemeral: true
                            });
                            return;
                        }
                        ...
                        await interaction.reply({
                            embeds: [{
                                title: `📁 Code Files for ${targetUser.username}`,
                        ...
                        if (deleted) {
                            await interaction.reply({
                                content: `✅ Successfully deleted **${filename}** from **${targetUser.username}**'s coding environment.`,
                                ephemeral: true
                            });
                        } else {
                            await interaction.reply({
                                content: `❌ File **${filename}** not found for user **${targetUser.username}**.`,
                                ephemeral: true
                            });
                        }
                    }
